...
TypeWriter.prototype.type = function() {
	var elText = this.element.innerHTML;
	if(this.delta == 0 || this.isPaused) {
		this.delta = 200;
		this.isPaused = false;
	}
	if(!this.deleting) {
		elText = this.element.innerHTML;
		...
		if(!this.isPaused)
			this.element.innerHTML = this.messageToDisplay.substring(0, elText.length + 1);
